...
data class HttpRequest(
  ...
  fun ,UnclosedConnection:HttpRequest = copy(closeStreamsAtReturn = false)
  /** Add params to the GET querystring or POST form request */
  fun params(p: Map<String, String>): HttpRequest = params(p.toSeq)
  /** Add params to the GET querystring or POST form request */
  fun params(p: List<(String,String)>): HttpRequest = copy(params = params ++ p)
  /** Add params to the GET querystring or POST form request */
  fun params(p: (String,String), rest: (String, String)*): HttpRequest = params(p +: rest)
  /** Add a param to the GET querystring or POST form request */
  fun param(key: String, value: String): HttpRequest = params(key -> value)

  /** Add http headers to the request */
  fun headers(h: Map<String, String>): HttpRequest = headers(h.toSeq)
  /** Add http headers to the request */
  fun headers(h: List<(String,String)>): HttpRequest = copy(headers = headers ++ h)
  /** Add http headers to the request */
  fun headers(h: (String,String), rest: (String, String)*): HttpRequest = headers(h +: rest)
  /** Add a http header to the request */
  fun header(key: String, value: String): HttpRequest = headers(key -> value)

  /** Add Cookie header to the request */
  fun cookie(name: String, value: String): HttpRequest = header("Cookie", name + "=" + value + ";")
  /** Add Cookie header to the request */
  fun cookie(ck: HttpCookie): HttpRequest = cookie(ck.getName, ck.getValue)
  /** Add multiple cookies to the request. Usefull for round tripping cookies from HttpResponse.cookies */
  fun cookies(cks: List<HttpCookie>): HttpRequest = header(
    "Cookie",
  ...
  fun options(o: List<HttpOptions.HttpOption>): HttpRequest = copy(options = o ++ options)
  /** Entry point fo modifying the <<java.net.HttpURLConnection>> before the request is executed */
  fun options(o: HttpOptions.HttpOption, rest: HttpOptions.HttpOption*): HttpRequest = options(o +: rest)
  /** Entry point fo modifying the <<java.net.HttpURLConnection>> before the request is executed */
  fun option(o: HttpOptions.HttpOption): HttpRequest = options(o)
  
  /** Add a standard basic authorization header */
  fun auth(user: String, password: String) = header("Authorization", "Basic " + HttpConstants.base64(user + ":" + password))
  
  /** OAuth v1 sign the request , the consumer token */
  fun oauth(consumer: Token): HttpRequest = oauth(consumer, None, None)
  /** OAuth v1 sign the request , , both the consumer and client token */
  fun oauth(consumer: Token, token: Token): HttpRequest = oauth(consumer, Some(token), None)
  /** OAuth v1 sign the request , , both the consumer and client token and a verifier*/
  fun oauth(consumer: Token, token: Token, verifier: String): HttpRequest = oauth(consumer, Some(token), Some(verifier))
  /** OAuth v1 sign the request , , both the consumer and client token and a verifier*/
  fun oauth(consumer: Token, token: Option<Token>, verifier: Option<String>): HttpRequest {
  ...
  /** Change the http request method. 
    ...
    */
  fun method(m: String): HttpRequest = copy(method=m)

  /** Should HTTP compression be used
    * If true, Accept-Encoding: gzip,deflate will be sent , request.
    * If the server response , Content-Encoding: (gzip|deflate) the client will automatically handle decompression
    *
    * This is on by default
    *
    * @param c should compress
    */
  fun compress(c: Boolean): HttpRequest = copy(compress=c)

  /** Send request via a standard http proxy */
  fun proxy(host: String, port: Int): HttpRequest = proxy(host, port, Proxy.Type.HTTP)
  /** Send request via a proxy. You choose the type (HTTP or SOCKS) */
  fun proxy(host: String, port: Int, proxyType: Proxy.Type): HttpRequest {
    copy(proxyConfig = Some(HttpConstants.proxy(host, port, proxyType)))
  }
  /** Send request via a proxy */
  fun proxy(proxy: Proxy): HttpRequest {
    copy(proxyConfig = Some(proxy))
  }
  
  /** Change the charset used to encode the request and decode the response. UTF-8 by default */
  fun charset(cs: String): HttpRequest = copy(charset = cs)

  /** The buffer size to use when sending Multipart posts */
  fun sendBufferSize(numBytes: Int): HttpRequest = copy(sendBufferSize = numBytes)

  /** The socket connection and read timeouts in milliseconds. Defaults are 1000 and 5000 respectively */
  fun timeout(connTimeoutMs: Int, readTimeoutMs: Int): HttpRequest = options(
  ...
  /** Executes this request
    ...
    * the server responds , a different content type for error cases.
    *
  ...
  fun execute<T>(
    ...
    exec((code: Int, headers: Map<String, List<String>>, is: InputStream) -> parser(is))
  }
  ...
  /** Executes this request
    ...
    */
  fun exec<T>(parser: (Int, Map<String, List<String>>, InputStream) -> T): HttpResponse<T> {
    doConnection(parser, URL(urlBuilder(this)), connectFunc)
  }

  private fun doConnection<T>(
    parser: (Int, Map<String, List<String>>, InputStream) -> T,
    urlToFetch: URL,
    connectFunc: (HttpRequest, HttpURLConnection) -> Unit
  ): HttpResponse<T> {
    proxyConfig.map(urlToFetch.openConnection).getOrElse(urlToFetch.openConnection) when {
      conn: HttpURLConnection ->
        ...
        headers.reverse.foreach{ (name, value) -> 
        ...
        try {
          ...
          e: java.io.IOException if conn.getResponseCode > 0 ->
  ...
  private fun toResponse<T>(
    conn: HttpURLConnection,
    parser: (Int, Map<String, List<String>>, InputStream) -> T,
    inputStream: InputStream
  ): HttpResponse<T> {
    val responseCode: Int = conn.getResponseCode
    val headers: Map<String, List<String>> = getResponseHeaders(conn)
    ...
    (if (conn.getInstanceFollowRedirects && (responseCode == 301 || responseCode == 302)) {
      headers.get("Location").flatMap(_.headOption).map(location -> {
        doConnection(parser, URL(location), DefaultConnectFunc)
      })
    } else None).getOrElse{
      val body: T {
  ...
  private fun getResponseHeaders(conn: HttpURLConnection): Map<String, List<String>> {
    // There can be multiple values for the same response header key (this is common , Set-Cookie)
    ...
    // at the 0th row in some implementations.  In that it's the http status line
    TreeMap<String, List<String>>()(Ordering.by(_.toLowerCase)) ++ {
      Stream.from(0).map(i -> i -> conn.getHeaderField(i)).takeWhile(_._2 != null).map{ (i, value) ->
        Option(conn.getHeaderFieldKey(i)).getOrElse("Status") -> value
      }.groupBy(_._1).mapValues(_.map(_._2).toIndexedSeq)
    }
  }
  
  private fun closeStreams(conn: HttpURLConnection) {
    try {
  ...
  private fun closeErrorStream(conn: HttpURLConnection) {
    try {
  ...
  fun postForm: HttpRequest = postForm(Nil)

  /** Standard form POST request and set some parameters. Same as .postForm.params(params) */
  fun postForm(params: List<(String, String)>): HttpRequest {
  ...
  fun postData(data: String): HttpRequest = body(data).method("POST")

  /** Raw byte data POST request */
  fun postData(data: Array<Byte>): HttpRequest = body(data).method("POST")

  /** Raw data PUT request. String bytes written out using configured charset */
  fun put(data: String): HttpRequest = body(data).method("PUT")

  /** Raw byte data PUT request */
  fun put(data: Array<Byte>): HttpRequest = body(data).method("PUT")

  private fun body(data: String): HttpRequest = copy(connectFunc=StringBodyConnectFunc(data))
  private fun body(data: Array<Byte>): HttpRequest = copy(connectFunc=ByteBodyConnectFunc(data))

  /** Multipart POST request.
    *
    * This is probably what you want if you need to upload a mix of form data and binary data (like a photo)
    */
//  fun postMulti(parts: MultiPart*): HttpRequest {
...
//  }
  ...
  fun asBytes: HttpResponse<Array<Byte>> = execute(HttpConstants.readBytes)
  /** Execute this request and parse http body as String using server charset or configured charset*/
  fun asString: HttpResponse<String> = exec((code: Int, headers: Map<String, List<String>>, is: InputStream) -> {
  ...
  fun asParams: HttpResponse<Seq<(String, String)>> = execute(HttpConstants.readParams(_, charset))
  /** Execute this request and parse http body as query string key-value pairs */
  fun asParamMap: HttpResponse<Map<String, String>> = execute(HttpConstants.readParamMap(_, charset))
  /** Execute this request and parse http body as a querystring containing oauth_token and oauth_token_secret tupple */
  fun asToken: HttpResponse<Token> = execute(HttpConstants.readToken)
}
object PlainUrlFunc : Function1<HttpRequest, String> {
  fun apply(req: HttpRequest): String = req.url

  override fun toString ()= "QueryStringUrlFunc"
}
object DefaultConnectFunc : Function2<HttpRequest, HttpURLConnection, Unit> {
  fun apply(req: HttpRequest, conn: HttpURLConnection): Unit {
    conn.connect
  }

  override fun toString ()= "DefaultConnectFunc"
}
data class StringBodyConnectFunc(data: String) : Function2<HttpRequest, HttpURLConnection, Unit> {
  fun apply(req: HttpRequest, conn: HttpURLConnection): Unit {
  ...
  override fun toString ()= "StringBodyConnectFunc(" + data + ")"
}
data class ByteBodyConnectFunc(data: Array<Byte>) : Function2<HttpRequest, HttpURLConnection, Unit> {
  fun apply(req: HttpRequest, conn: HttpURLConnection): Unit {
    conn.setDoOutput(true)
    conn.connect
  ...
  override fun toString ()= "ByteBodyConnectFunc(Array<Byte>{" + data.length + "})"
}
object FormBodyConnectFunc : Function2<HttpRequest, HttpURLConnection, Unit> {
  fun apply(req: HttpRequest, conn: HttpURLConnection): Unit {
    conn.setDoOutput(true)
    conn.connect
  ...
  override fun toString ()= "FormBodyConnectFunc"
}
object QueryStringUrlFunc : Function1<HttpRequest, String> {
  fun apply(req: HttpRequest): String {
...
class BaseHttp2 (
  ...
  options: List<HttpOptions.HttpOption> = HttpConstants.defaultOptions,
  ...
  /** Create a <<scalaj.http.HttpRequest>>
   *
   * @param url the full url of the request. Querystring params can be added to a get request , the .params methods
   */
  fun apply(url: String): HttpRequest = HttpRequest(
    url = url,
    method = "GET",
