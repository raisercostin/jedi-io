...
case class EmailLocation(host:String, port:Int, username:String, password:String) extends NavigableInOutLocation { self =>
 ...
 */
  ...
  def append: Boolean = ???
  protected def unsafeToOutputStream: java.io.OutputStream = ???
  def withAppend: Repr = ???
}